...
define(function(require, exports, module) {
     ...
     *
     * @example
     *     Designer.registerTheme('default', {
     *         'root-color': 'red',
     *         'root-stroke': 'none',
     *         'root-padding': [10, 20]
     *     });
    ...
    kity.extendClass(Designer, {
        ...
        getTheme: function(node) {
            return this._theme || this.getOption('defaultTheme') || 'classic';
        },
        ...
        getNodeStyle: function(node, name) {
            var value = this.getStyle(node.getData('outlineType') + '-' + name, node);
            if(value === null){
              value = this.getStyle('node-' + name, node);
            }
    ...
    kity.extendClass(DesignerNode, {
        getStyle: function(name) {
            return this.getDesigner().getNodeStyle(this, name);
        }
    });
    ...
    Module.register('Theme', {
        ...
        commands: {
             ...
             *    允许使用的主题可以使用 `witdesigner.Designer.getThemeList()` 查询
             * @state
             *   0: 始终可用
             * @return 返回当前的主题名称
             */
